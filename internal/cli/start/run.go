...
func runStart(cmd *cobra.Command, opts Options, tunnelFlag, publicURLFlag, logLevelFlag string, background, bgChild, subAgentRouter, compress bool) error {
	...
	if bgChild {
		return serveWithWatchdog(opts.Version, dataRoot, settings, subAgentRouter, compress)
	}
	return serveGateway(opts.Version, dataRoot, settings, subAgentRouter, compress)
}
