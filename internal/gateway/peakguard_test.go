...
func TestOpenRouterModelFor(t *testing.T) {
	tests := []struct {
		...
		{"kimi-k3", "kimi-k3"},
		{"deepseek-flash", config.ModelOpenRouterDeepSeekV4Flash},
...
func TestPeakNow(t *testing.T) {
	tests := []struct {
		...
		{"deepseek flash sunday off-peak", "deepseek-flash", peakTime(2, time.Sunday), false},
		{"deepseek flash sunday post-cutover off-peak", "deepseek-flash", time.Date(2026, 8, 23, 7, 0, 0, 0, time.UTC), false},
		{"deepseek flash saturday post-cutover off-peak", "deepseek-flash", time.Date(2026, 8, 29, 7, 0, 0, 0, time.UTC), false},
...
func TestApplyPeakReroute(t *testing.T) {
	...
	tests := []struct {
		...
		{"deepseek small peak with key", withKey(), "deepseek-flash", peakTime(8, time.Tuesday), config.ModelOpenRouterDeepSeekV4Flash, true},
...
func TestOpenRouterOnlyDuringPeak(t *testing.T) {
	...
	t.Run("guard corrects DeepSeek OpenRouter id when not forced", func(t *testing.T) {
		s := withKey()
		if peakNow("deepseek-flash", time.Now()) {
			t.Skip("test run during real DeepSeek peak hours; guard check non-deterministic")
		}
		if got := s.isPeakAllowedOpenRouter(config.ModelOpenRouterDeepSeekV4Flash, "req"); got != config.ModelDeepSeekFlash {
			t.Fatalf("guard returned %q, want corrected %s", got, config.ModelDeepSeekFlash)
		}
	})
	...
	t.Run("openRouterTwinFor maps real models", func(t *testing.T) {
		if twin, ok := openRouterTwinFor(config.ModelDeepSeekFlash); !ok || twin != config.ModelOpenRouterDeepSeekV4Flash {
